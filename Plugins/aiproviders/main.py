...
Opens AI chat providers from the flashlight: `!chatgpt <query>` etc. Bang
dispatch arrives as perform kind "command" with the bang token as the
subcommand. The optional query is pre-filled via the provider's `q` URL
parameter, and a Return keystroke is synthesized after a load delay so the
prompt actually sends. Best-effort — if focus lands elsewhere the keystroke
goes there, the same tradeoff the Rust implementation made.
"""
import subprocess
import sys
import time
...
from flashplugin import Plugin, fail, ok  # resolved via host-injected PYTHONPATH
...
def on_command(params):
    ...
    if provider is None:
        return fail(f"unknown ai provider: !{bang}")
    ...
    if not opened.get("ok"):
        return fail(opened.get("error") or "host.open failed")
    if query:
...
if __name__ == "__main__":
    plugin.serve(on_command=on_command)
