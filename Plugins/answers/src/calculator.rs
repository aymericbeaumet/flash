//! Arithmetic, unit and currency answers: fend over a warm ECB rate snapshot.
...
pub(crate) struct Calculator {
    rates: RatesStore,
    target_currencies: RwLock<Vec<String>>,
}

impl Calculator {
    pub(crate) async fn start(&self, ctx: Context) {
        if let Ok(mut targets) = self.target_currencies.write() {
            *targets = normalize_targets(ctx.config_json("target_currencies"));
        }
        rates::seed_and_refresh(ctx, self.rates.clone()).await;
    }
}

impl Engine for Calculator {
    fn answers(&self, query: &str) -> Vec<QueryAnswer> {
        let targets = self
            ...
            .unwrap_or_default();
        evaluate(query, &self.rates, &targets)
    }
}
...
fn evaluate(raw_query: &str, rates: &RatesStore, target_currencies: &[String]) -> Vec<QueryAnswer> {
