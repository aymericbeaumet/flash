...
- A user-configured mapping to `enter_insert_mode` / `enter_locked_insert_mode` (or an explicit invocation of those verbs). There are no built-in `i`, `I`, `a`, `A`, `o`, or `O` insert shortcuts, and `app_find` / `tab_new` no longer enter INSERT.
