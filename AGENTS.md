...
**Flashlight tie-break ordering.** When match scores are close, `CandidateFinder.sortedMatches` settles ties by source tier (in `sourcePrecedenceTierIndex`): tmux windows > browser tabs > **active** apps (pid set) > **inactive** apps > the rest (slack, notes, reminders, contacts, other plugins). The tier check runs before the generic alive/dead check so a running app can't leapfrog a tmux window or browser tab. Strong content matches (score gap ≥ `aliveTieBreakScoreMargin`) still win outright regardless of tier.
...
- A committed `f` or `F` click hint, including modified, right-click, and double-click commits; these are explicit mouse interactions by the user.
