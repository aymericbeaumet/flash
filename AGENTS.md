...
nonpersistent session until dismissal. Named sessions restart after exit:
...
a named nonpersistent window merely because its child exited. Dismissal/release,
...
for eager startup/reuse. Unnamed `terminal_show` shells are one-shot: hiding,
closing, exiting, failing, or killing one removes its registry entry, popup,
and scrollback permanently. Explicit restart
remains available while open. Release sessions after presentation becomes hidden
to avoid recursive registry dismissal. Standalone windows survive statusbar hiding
...
Preserve screen clamping and restore the previous app on explicit close or terminal termination,
never after focus loss to another app. Use `absolute-centre` for screen-centered labels; native `centre`
...
cleanup: named sessions restart with backoff and keep their popup; unnamed
shells end permanently. Invalidate input generations before stopping so delayed
input cannot reach a replacement. Dismissal retains the declared persistence
policy. Inherited exit mappings come only from winning INSERT-active
`enter_normal_mode` or `leave_mode` bindings; other global mappings are suspended. Returning
...
An all-scope mapping to `enter_normal_mode` or `leave_mode` enables advanced mode. Scoped mappings alone do not enable it. Status-bar visibility remains independently controlled by `statusbar.enabled`.
...
source metadata; the host does not use it to change modes.

INSERT remains active across focus and application changes until an explicit
exit mapping/verb. `leave_mode` exits INSERT to NORMAL and restores the saved
mode when closing command or terminal surfaces. Restoring a saved INSERT mode
is allowed. Enabling advanced mode starts NORMAL; disabling it enters the
separate disabled state. See `docs/normal-mode.md` for the maintained contract.
...
Unsupported commands/subcommands must produce the existing error toast and a
warning log; CLI rejection also returns a nonzero status. Preserve diagnostic
context for invalid mappings instead of silently dropping argv tokens.

## Status update continuity

Keep ordinary status updates in place and without animations. Planned resident
plugin reloads retain published segments for at most 10 seconds; explicit stop,
error, removal, and configuration replacement clear them. Dev builds stage binaries outside watched plugin roots and preserve unchanged
executables (including signing identity) so no-op builds do not restart plugins.
Only `#[cyc]` content opts into upward carousel transitions; clear those transitions when a pooled
layer is reused for ordinary metrics. See `docs/status-plugins.md`.
