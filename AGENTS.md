...
1. The `flash` CLI runs (`flash mouse_target [secondary=1|double=1|middle=1|triple=1|move=1|drag=1] [modifiers=cmd+…]`, `flash mouse_grid [secondary=1|double=1|middle=1|triple=1|move=1|drag=1] [modifiers=cmd+…]`), or a configured `[mode.*]` mapping fires a pre-resolved `MappingAction`. The CLI sends a custom AppleEvent (class `Flsh`, ID `Cmd `); mappings dispatch the same `URLCommand` value directly.
