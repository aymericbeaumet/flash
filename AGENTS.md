...
9. Default keyboard shortcuts must not enter INSERT (`a/A/i/I/o/O/gi` included).
   `enter_insert_mode`, `focus_input` and unmapped-key/modifier passthrough are
   explicit configuration choices. App activation or editable focus alone never
   changes NORMAL. See the mode document for deliberate pointer/target commits.
