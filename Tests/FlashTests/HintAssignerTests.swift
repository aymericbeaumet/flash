...
final class HintAssignerTests: XCTestCase {
    ...
    func testUniformLength() {
        ...
        XCTAssertTrue(nine.allSatisfy { $0.count == 2 }, "9 items in 8-char alphabet → all 2-char labels")

        // count = 8 → exactly fits in 1-char labels.
        let eight = HintAssigner.generateLabels(count: 8, alphabet: alphabet)
        XCTAssertTrue(eight.allSatisfy { $0.count == 1 }, "8 items in 8-char alphabet → all 1-char labels")

        // count = 65 → 1 and 2 chars aren't enough (8^2 = 64), need 3.
        let many = HintAssigner.generateLabels(count: 65, alphabet: alphabet)
        XCTAssertTrue(many.allSatisfy { $0.count == 3 }, "65 items in 8-char alphabet → all 3-char labels")
    }
    ...
    func testHandAlternationPrefersAlternatingPairs() {
        ...
        let labels = HintAssigner.generateLabels(count: 16, alphabet: alpha, leftHand: leftHand, minLength: 2)
