...
///     and browser in-page DOM via AXWebArea descendants.
///
/// Within a chain, higher priority runs first. On overlapping rects the
/// higher-priority provider wins via spatial dedup in AppMonitor — so
/// tmux per-word rects suppress the AX text-area rect for terminals that
/// expose one (Terminal.app, iTerm2).
///
