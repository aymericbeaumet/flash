  ...
  func performAction(
    ...
    case .tabSelect(let index):
      ...
      BrowserTabSources.runAppleScriptAsync(script) { result in
        // App matched (activationPolicy gated us to Safari) and the script
        // ran — a non-"ok" result is the source's claim failing, not an
        // unrelated context. Return `.failed` so the host doesn't
