...
/// A native surface (a context menu, or an in-flight pointer→passthrough handoff) owns
...
struct RecaptureSuppression: Equatable {
  ...
  /// A pointer click is being probed for a passthrough-mode handoff.
  var pointerPassthroughHandoffUntil: Date?
  ...
  func pointerPassthroughHandoffActive(now: Date) -> Bool {
    Self.active(pointerPassthroughHandoffUntil, now: now)
  }
  ...
  func anyActive(now: Date) -> Bool {
    menuBarActive(now: now)
      ...
      || pointerPassthroughHandoffActive(now: now)
  }
  ...
  mutating func pruneExpired(now: Date) {
    ...
    if let until = pointerPassthroughHandoffUntil, until <= now {
      pointerPassthroughHandoffUntil = nil
    }
  }
