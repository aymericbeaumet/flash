...
final class FlashStatusBarController {
  ...
  private var sourceJobs: [String: StatusFormatCommandJob] = [:]
  private var shellJobs: [String: StatusFormatCommandJob] = [:]
  private var sourceDue: [String: TimeInterval] = [:]
  private var shellDue: [String: TimeInterval] = [:]
  private var shellCommands: [String: String] = [:]
  private var shellStarted: [String: TimeInterval] = [:]
  private var shellUpdated: Set<String> = []
  private var dynamicValues: [String: String] = [:]
  private var jobValues: [String: String] = [:]
  ...
  init(
    overlay: OverlayPanel, template: FlashStatusBarTemplate,
    popupTemplates: [String: FlashStatusBarTemplate] = [:],
    options: [String: String] = [:],
  ...
  func updateTemplate(
    ...
    queue.async { [weak self] in
      ...
      self.generation &+= 1
      self.stopOwnedJobs()
      self.sourceJobs.removeAll()
      self.shellJobs.removeAll()
      self.sourceDue.removeAll()
      self.shellDue.removeAll()
      self.template = template
      if let popupTemplates { self.popupTemplates = popupTemplates }
      if let options { self.options = options }
      if let sources {
        let unchanged = Set(sources.keys.filter { sources[$0] == self.sources[$0] })
        self.dynamicValues = self.dynamicValues.filter { unchanged.contains($0.key) }
        self.cycles = self.cycles.filter { unchanged.contains($0.key) }
        self.sources = sources
      }
      if let terminalPopupNames { self.terminalPopupNames = terminalPopupNames }
      if let refreshIntervalSeconds { self.refreshIntervalSeconds = refreshIntervalSeconds }
      self.publishCurrentModel()
    }
  }
  ...
  private func stopOwnedJobs() {
    let remaining = StatusFormatCommandJob.shutdown(
      Array(sourceJobs.values) + Array(shellJobs.values))
  ...
  private func publishCurrentModel() {
    ...
    for (name, cycle) in cycles { values[name] = "#[cyc]" + cycle.visibleLine + "#[nocyc]" }
  ...
  private func runDueJobs(now: TimeInterval) {
    for name in requiredSources.sorted() {
      guard let definition = sources[name], sourceJobs[name] == nil,
        sourceDue[name, default: -.infinity] <= now
      else { continue }
      sourceDue[name] =
        definition.intervalSeconds > 0 ? now + definition.intervalSeconds : .infinity
      startSource(name, definition: definition)
    }
    for key in requiredJobs.keys.sorted() {
      guard let request = requiredJobs[key] else { continue }
      if shellCommands[key] != request.command {
        if let job = shellJobs.removeValue(forKey: key) {
          StatusFormatCommandJob.shutdown([job], graceSeconds: 0)
        }
        shellDue[key] = -.infinity
      }
      guard shellJobs[key] == nil, shellDue[key, default: -.infinity] <= now else { continue }
      shellDue[key] = refreshIntervalSeconds > 0 ? now + max(1, refreshIntervalSeconds) : .infinity
      startShell(request)
    }
  }

  private func startSource(_ name: String, definition: FlashStatusBarSourceDefinition) {
    let generation = generation
    let base = FlashProcessEnvironment.shared.environment
    var environment = base
    for (key, value) in definition.environment {
      environment[key] = StatusTerminalRegistry.expand(value, environment: base)
    }
    var argv = definition.command.map {
      StatusTerminalRegistry.expand($0, environment: environment)
    }
    if let executable = argv.first {
      argv[0] = Self.executablePath(executable, environment: environment)
    }
    let directory = definition.workingDirectory.map {
      StatusTerminalRegistry.expand($0, environment: environment)
    }
    do {
      sourceJobs[name] = try StatusFormatCommandJob(
        queue: queue, argv: argv, environment: environment, workingDirectory: directory,
        timeoutSeconds: definition.timeoutSeconds, onLine: { _ in },
        onCompletion: { [weak self] status, output in
          guard let self, generation == self.generation else { return }
          self.sourceJobs[name] = nil
          if status == 0, !output.trimmed.isEmpty {
            if let period = definition.cycleIntervalSeconds {
              let lines = output.split(separator: "\n").map { String($0).trimmed }.filter {
                !$0.isEmpty
              }
              if var cycle = self.cycles[name] {
                cycle.refresh(lines: lines, periodSeconds: period, now: Self.uptime)
                self.cycles[name] = cycle
              } else {
                self.cycles[name] = FlashStatusBarCycleState(
                  lines: lines, periodSeconds: period, now: Self.uptime)
              }
            } else {
              self.dynamicValues[name] = output.trimmed
            }
          }
          self.publishCurrentModel()
        })
  ...
  private func startShell(_ request: StatusFormatJobRequest) {
    let generation = generation
    let key = request.rawCommand
    shellCommands[key] = request.command
    shellStarted[key] = Self.uptime
    shellUpdated.remove(key)
    do {
      shellJobs[key] = try StatusFormatCommandJob(
        queue: queue, argv: ["/bin/sh", "-c", request.command],
        environment: FlashProcessEnvironment.shared.environment,
        onLine: { [weak self] line in
          guard let self, generation == self.generation else { return }
          self.shellUpdated.insert(key)
          if self.jobValues[key] != line {
            self.jobValues[key] = line
            self.scheduleJobPublish()
          }
        },
        onCompletion: { [weak self] _, _ in
          guard let self, generation == self.generation else { return }
          self.shellJobs[key] = nil
          self.shellStarted[key] = nil
          if !self.shellUpdated.contains(key), self.jobValues[key] != "" {
            self.jobValues[key] = ""
            self.scheduleJobPublish()
          }
          self.armTimer()
      ...
      jobValues[key] = "<'\(key)' didn't start>"
      shellStarted[key] = nil
      scheduleJobPublish()
  ...
  private func armTimer() {
    ...
    timer = nil
    guard started else { return }
    var dates = requiredSources.compactMap { sourceJobs[$0] == nil ? sourceDue[$0] : nil }
    dates += requiredJobs.keys.compactMap { shellJobs[$0] == nil ? shellDue[$0] : nil }
    dates += requiredJobs.keys.compactMap {
      jobValues[$0] == nil ? shellStarted[$0].map { $0 + 2 } : nil
    }
    dates += cycles.values.filter(\.needsRotationTimer).map(\.nextRotationAt)
    ...
    timer.schedule(deadline: .now() + max(0.001, next - Self.uptime), leeway: .milliseconds(25))
  ...
  private func tick() {
    let now = Self.uptime
    for key in requiredJobs.keys where jobValues[key] == nil {
      if let start = shellStarted[key], now - start >= 2 { jobValues[key] = "<'\(key)' not ready>" }
    }
    ...
    for name in Array(cycles.keys) { _ = cycles[name]?.advanceIfDue(now: now) }
    publishCurrentModel()
  }
