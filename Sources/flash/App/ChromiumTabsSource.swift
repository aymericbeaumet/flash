  ...
  func performAction(
    ...
    case .tabSelect(let index):
      ...
      BrowserTabSources.runAppleScriptAsync(script) { result in
        // App matched (activation policy gated to Chromium-family) and the
        // script ran — non-"ok" is the source's claim failing, not an
        // unrelated context. `.failed` so the host doesn't keystroke-fall
        // back on top of it.
    ...
    case .tabClose:
      // Closing the last tab via `close active tab` collapses to closing the
      // window — same as the user pressing ⌘W natively, which is what we
      // want: the gesture stays "close this thing in this context".
      let script = """
        tell application \(BrowserTabSources.scriptString(appName))
