...
///
/// The app is headless, so diagnostics go to stderr. In the installed app
/// launch path stderr is collected under ~/Library/Logs/Flash/.
final class FlashProfiler {
    ...
    func finish(outcome: String, detail: String? = nil) {
        ...
        var header = "flash: profile kind=\(kind) id=\(id) total_ms=\(Self.format(total)) outcome=\(Self.clean(outcome))"
        if let detail, !detail.isEmpty {
        ...
        for mark in snapshot {
            var line = "flash: profile kind=\(kind) id=\(id) elapsed_ms=\(Self.format(mark.elapsedMs)) step_ms=\(Self.format(mark.stepMs)) stage=\(Self.clean(mark.name))"
