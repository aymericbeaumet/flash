...
/// The fixture page Firefox loads for E2E AX coverage. Shared by the
/// standalone `flash-firefox-e2e` runner and the opt-in
/// `FirefoxIntegrationTests` xctest target so both forms exercise the
/// exact same HTML against the exact same assertions.
///
